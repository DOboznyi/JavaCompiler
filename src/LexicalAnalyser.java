...
public class LexicalAnalyser
{
    ...
    recrdKWD[] tablKWD = new recrdKWD[] {
        ...
        new recrdKWD(oprtr[tokType._or.ordinal()].toCharArray(), tokType._or,1)
};
    ...
    {//static int lxNmb=0;
        ...
        do {
            ...
            if (((sP == autStat.S2) || (sP == autStat.S3)) && ((c.ordinal() > ltrType.nc.ordinal()) && (c.ordinal() < ltrType.dlmeormr.ordinal())))
            {// пошук в таблиці групових роздільників
                ...
                if (pRt != null)
                {
                    ...
                    {
                        s = autStat.Scr; nodes[nNode].start = nImCr;
                        continue;
                    }
                    if (pRt.func == tokType._remL)
                    {
        ...
        switch (sP)
        {
            ...
            case S0:
                ...
                {
                    cntMdB++; nodes[nNode].ndOp = tokType._tdbr;
                }
                if (nodes[nNode].ndOp == tokType._ocbr && nodes[nNode].ndOp == tokType.values()[cntMdB])
                {
                    nodes[nNode].ndOp = tokType._tcbr; cntMdB--;
                }
                if (nodes[nNode].ndOp != tokType._nil)
                //		&&imgBuf[nImBg]==)
                {
                    ...
                    //		 if(dlCdsC[l0]!=_nil||ltClsC[l0]==dlmaux||ltClsC[l0]==dlmeormr)
                    {
                        ...
                        {
                            nImCr--;
                            imgBuf[nImBg] = imgBuf[nImCr]; nImCr = nImBg + 1;
                        }// 04.07.07
                        else nImCr = nImBg;
                    }
                    return nNode++;
            ...
            case S2s:   // формування констант
            {
                ...
                else { eNeut(nNode); }      // фіксація помилки
    ...
    int cmpStr(char[] s1, char[] s2, int start)
    {
        int n = 0;
        int k = start;
        while ((s1[n] == s2[k]) && (s1[n] != 0)) { n++; k++; if (n != s1.length || k != s2.length) { break; } }
        return s1[n] - s2[k];
    }
